...
class Reseau
{
    ...
    public:
        ...
        bool verifierListeInterdite(const std::vector<int>& m_listeinterdite,const int id)
        {
            for(int i=0; i<m_listeinterdite.size();++i)
            {
                if(id==m_listeinterdite[i])
                    return true;
            }
            return false;
        }

        void ajouterListeInterdite(std::vector<int> & m_listeinterdite,const int id)
        {
            /// on n'ajoute pas la station si elle y est déjà
            if(verifierListeInterdite(m_listeinterdite,id))
               return;
            /// si retourne faux on ajoute à la liste des interdits
            m_listeinterdite.push_back(id);
        }

        void attribuerWelshPowell(int& nbFrequences)
        {
            int frequence=1;
            std::vector<int> m_listeinterdite;
           ...
           ///Parcours des stations à partir de 0 /// PB ICI je pense car la fréquence n'est pas différente
           for(int i=0; i<m_stations.size();++i)
           {
               /// si la fréquence est différente de 0 alors on passe à la station suivante
               if(m_stations[i]->getFrequence()!=0)
               {
                   continue; /// on remonte au for
               }

               /// si la fréquence vaut 0 alors elle prend la valeur de la nouvelle fréquence
               m_stations[i]->setFrequence(frequence);

               /// On ajoute les adjacents des stations dans la liste des interdits
               for (const auto a : m_stations[i]->getAdjacent())
               {
                       ajouterListeInterdite(m_listeinterdite,a->getId());
               }

               ///parcours de la suite de la liste
               for(int j=i+1; j<m_stations.size();++j)
               {
                   bool interdit;

                   /// si la fréquence est différente de 0 alors on passe à la station suivante
                   if(m_stations[j]->getFrequence()!=0)
                   {
                        continue; /// on remonte au for
                   }

                   /// on vérifie si la station suivante fait partie de la liste des interdits
                   interdit=verifierListeInterdite(m_listeinterdite,m_stations[j]->getId());
                   ...
                   /// si la station ne fait pas partie de la liste des interdits alors elle prend la même fréquence
                   m_stations[j]->setFrequence(frequence);

                   /// On ajoute les adjacents des stations dans la liste des interdits
                   for (const auto a : m_stations[j]->getAdjacent())
                   {
                       ajouterListeInterdite(m_listeinterdite,a->getId());
                   }
               }
               ///La fréquence augmente : nouvelle fréquence
               frequence++;
               /// on efface la liste interdite.
               m_listeinterdite.clear();
           }
           nbFrequences=(frequence-1);
        }
